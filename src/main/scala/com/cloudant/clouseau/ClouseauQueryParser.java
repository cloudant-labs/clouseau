...
public final class ClouseauQueryParser extends QueryParser {
    ...
    private static final String fpRegex = ("[\\x00-\\x20]*" + "[+-]?(" + "NaN|"
            + "Infinity|" + "((("
            + Digits
            + "(\\.)?("
            + Digits
    ...
    public ClouseauQueryParser(final Version version,
            final String defaultField, final Analyzer analyzer) {
        super(version, defaultField, analyzer);
    }

    @Override
    protected Query getRangeQuery(final String field, final String lower,
            final String upper, final boolean inclusive) throws ParseException {
        if (isNumber(lower) && isNumber(upper)) {
            return NumericRangeQuery.newDoubleRange(field, 8,
                    Double.parseDouble(lower), Double.parseDouble(upper),
                    inclusive, inclusive);
        }
    ...
    protected Query getFieldQuery(final String field, final String queryText,
        ...
        if (isNumber(queryText)) {
            return new TermQuery(new Term(field,
                    NumericUtils.doubleToPrefixCoded(Double
                            .parseDouble(queryText))));
        }
