...
class IndexManagerServiceSpec extends SpecificationWithJUnit {
  ...
  "the index manager" should {
    ...
    "open an index when asked" in new manager_service {
      node.call(service, OpenIndexMsg(mbox.self, "foo", AnalyzerOptions.fromAnalyzerName("standard"))) must beLike { case ('ok, pid: Pid) => ok }
    }

    "return the same index if it's already open" in new manager_service {
      node.call(service, OpenIndexMsg(mbox.self, "foo", AnalyzerOptions.fromAnalyzerName("standard"))) match {
        case ('ok, pid) =>
          node.call(service, OpenIndexMsg(mbox.self, "foo", AnalyzerOptions.fromAnalyzerName("standard"))) must be equalTo ('ok, pid)
      }
    }
