...
All tools and various configuration options are managed in [`mise.toml`](mise.toml).

Additional dependencies you may need to install manually on macOS:

- [Homebrew](https://brew.sh/)
- [mise](https://mise.en.dev/) via `brew install mise`
...
If you don't have `mise` on your system, there are extra steps need to be done.
The steps are documented in full details [here](scripts/bootstrap.md).
Essentially the steps are:

1. Install [`mise`](https://github.com/jdx/mise) with e.g. `brew install mise`
2. Parse the project config file [`mise.toml`](mise.toml) with `mise trust`
3. Verify your OS has all tools using `cli verify`
4. Use step-by-step guide script to finish installation (you might need to call it multiple times) `cli bootstrap`
...
## Monitoring metrics using `jconsole`
...
## Recommendations for development using `sbt`

The Scala build tool `sbt` can be used directly to compile and launch a service, or to run individual unit tests, e.g.:

```bash
# Compile and start the clouseau node
$ sbt run
